...
export class RuleEvaluator implements INodeType {
  ...
  private evaluateItem(
    ...
    const resolveOutputField =
      RuleEvaluator.prototype.resolveOutputField.bind(this);
    const createOutputItem =
      RuleEvaluator.prototype.createOutputItem.bind(this);
  ...
  description: INodeTypeDescription = {
    displayName: "Rule Enricher",
    name: "ruleEvaluator",
    icon: "file:rule-evaluator.svg",
    group: ["transform"],
    version: 1,
    description:
      "Legacy rule enricher for deterministic decisions. Deprecated: use Business Rules for switchless routing.",
    codex: {
      alias: ["Rule Evaluator", "Rule Enricher", "Policy Rules"],
    },
    ...
    hints: [
      {
        type: "warning",
        message: "Deprecated. Use Business Rules for routing without Switch.",
      },
    ],
    inputs: ["main"],
    outputs: ["main"],
